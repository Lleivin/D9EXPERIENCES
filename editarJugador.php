...
//RECIBIR LOS DATOS DE LA PAGINA ANTERIOR PARA REALIZAR EL CAMBIO
// Verificar si se ha recibido el parámetro DNI
if (isset($_GET['dni']) && !empty($_GET['dni'])) {
  // Escapar y almacenar el DNI recibido
  $dniJugadorAmodificar = mysqli_real_escape_string($conexion, $_GET['dni']);
  
  // Obtener datos del jugador utilizando la función
  $jugadorAmodificar = obtenerJugadorPorDNI($conexion, $dniJugadorAmodificar);

} else {
  // Mostrar mensaje de error si el DNI no ha sido proporcionado
  echo "<p style='color: red;'>No se ha especificado un jugador a modificar</p> <br> <a href='administracion.php'>Selecciona un jugador</a>";
  exit;
}
...
<div id="formularioModificar" class="cajaPerfil cajaperfilActualiza">
    <h3>Editar a: <?= htmlspecialchars($jugadorAmodificar['nombreJugador']) ?></h3>
    <form id="formAdministradorCambiarJugadores" class="cajaperfilActualiza" action="editarJugador.php?dni=<?= htmlspecialchars($jugadorAmodificar['dniJugador']) ?>" method="POST">
        <!-- Para saber a cuál DNI has hecho click -->
        <?php $jugadorSeleccionadoAdmin = $jugadorAmodificar['dniJugador'] ?>
        <input class="d-none" id="dniJugadorSeleccionadoAdmin" name="dniJugadorSeleccionadoAdmin" value="<?= $jugadorSeleccionadoAdmin ?>">

        <div class="cajaformulario">
            <label for="actualizaNombre">Nombre: </label>
            <input id="actualizaNombre" class="usuario" type="text" name="nuevoNombreJugador" value="<?= htmlspecialchars($jugadorAmodificar['nombreJugador']) ?>">
        </div>

        <div class="cajaformulario">
            <label for="actualizaApellido">Primer Apellido: </label>
            <input id="actualizaApellido" class="usuario" type="text" name="nuevoApellido1Jugador" value="<?= htmlspecialchars($jugadorAmodificar['apellido1Jugador']) ?>">
        </div>

        <div class="cajaformulario">
            <label for="actualizaApellido2">Segundo Apellido: </label>
            <input id="actualizaApellido2" class="usuario" type="text" name="nuevoApellido2Jugador" value="<?= htmlspecialchars($jugadorAmodificar['apellido2Jugador']) ?>">
        </div>

        <div class="cajaformulario">
            <label for="actualizaNick">Nick: </label>
            <input id="actualizaNick" class="usuario" type="text" name="nuevoNickJugador" value="<?= htmlspecialchars($jugadorAmodificar['nickJugador']) ?>">
        </div>

        <div class="cajaformulario">
            <label for="actualizaTelefono">Teléfono: </label>
            <input id="actualizaTelefono" class="usuario" type="tel" name="nuevoTelefonoJugador" value="<?= htmlspecialchars($jugadorAmodificar['telefonoJugador']) ?>">
        </div>
        ...
        <div id="cajaMensajeErrorModificarJugadorAdmin" class="d-flex flex-wrap col-12 col-sm-12 text-center pl-3 pr-3"></div>
