...
if (isset($_GET['idPartida'])) {
  ...
  echo "<p style='color: red;'>No se ha especificado una partida para modificar</p> <br> <a href='administracion.php'>Selecciona una partida</a>";
  exit;
}
...
<!-- ------------------EDITAR LA PARTIDA----------------- -->
<div class="cajaLogin" id="cajaNuevaPartida">
    
    <form id="formAdministradorCambiarPartidas" class="formLogin" action="" method="POST">
        <h4 class="d-flex m-auto col-12 col-sm-12">Edita la Partida</h4>
        ...
        <div class="cajaformulario">
            <label class="d-flex mb-10px" for="">Campo de juego<span>*</span> </label>
            <select id="campoJuego" class="selecciones" name="idCampo">
                <option value="" disabled>Selecciona un campo</option>
                <?php
                // Rellenar el <select> con los datos de la tabla campos
                if ($todosCampos->num_rows > 0) {
                    while ($campo = $todosCampos->fetch_assoc()) {
                        $selected = $campo['idCampo'] == $idCampo ? 'selected' : '';
                        echo '<option value="' . htmlspecialchars($campo['idCampo']) . '" ' . $selected . '>'
                             . htmlspecialchars($campo['nombreCampo']) . '</option>';
                    }
                } else {
        ...
        <div class="cajaformulario">
            <label for="">Nombre de la partida<span>*</span> </label>
            <input id="nombrePartida" class="usuario" type="text" name="nombrePartida"
                placeholder="Guerra Sucia" value="<?= htmlspecialchars($partidaAmodificar['nombrePartida']) ?>">
        </div>

        <div class="cajaformulario">
            <label for="">Fecha de la partida<span>*</span> </label>
            <input id="fechaPartida" class="usuario" type="date" name="fechaPartida" 
                value="<?= htmlspecialchars($partidaAmodificar['fecha']) ?>">
        </div>
        
        <div class="cajaformulario">
            <label for="horaPartida">Hora de la partida<span>*</span> </label>
            <input id="horaPartida" class="usuario" type="time" name="horaPartida"
                value="<?= htmlspecialchars($partidaAmodificar['horaPartida']) ?>">
        </div>

        <div class="cajaformulario">
            <label for="precioPartida">Precio<span>*</span></label>
            <input id="precioPartida" class="usuario" type="number" name="precioPartida"
                placeholder="15 €" step="0.50" value="<?= htmlspecialchars($partidaAmodificar['precioPartida']) ?>">
        </div>

        <div class="cajaformulario">
            <label for="maximoJugadoresPartida">Máximo de Jugadores<span>*</span></label>
            <input id="maximoJugadoresPartida" class="usuario" type="number" name="maximoJugadoresPartida"
                placeholder="Nº máximo de jugadores" step="1" value="<?= htmlspecialchars($partidaAmodificar['numeroJugadores']) ?>">
        </div>


        <div class="cajaformulario d-flex col-10">
            <label for="descripcionPartida">Descripción</label>
            <textarea id="descripcionPartida" class="selecciones" name="descripcionPartida" maxlength="200" rows="4" placeholder="Ligera descripción de la partida, tipo de juego y objetivos."><?= htmlspecialchars($partidaAmodificar['descripcionPartida']) ?></textarea>
        </div>

        <div class="cajaBotonForm">
            <input class="boton" type="submit" id="btn_crearPartida" name="entrar" value="Modificar">
        </div>

        <div id="cajaMensajeErrorModificarPartida" class="d-flex flex-wrap col-12 col-sm-12 text-center pl-3 pr-3"></div>
